...
An actual branching, 50-state-capable DFS tree sits directly below the map and remains synchronized with every displayed solver event. It connects each decision to its separate color options and descendant decisions inside a limited-height, two-directional scroll pane that automatically follows the active branch. The current DFS route is bold and colored by its choices, while the exact node or branch affected by the current step is outlined and tagged. Propagation, forced assignments, contradictions, and backtracks therefore remain visible even when no new decision node is created. A compact sticky monitor keeps remaining search space, solver status, propagation mode, and speed visible. Select any state to inspect its assignment, complete domain, and neighbors in an optional details sidebar; the sidebar is collapsed by default so the map-and-tree area can expand. Rejected and constraint-pruned color branches remain visible with crossed-out labels.
